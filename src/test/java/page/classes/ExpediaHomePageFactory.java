...
public class ExpediaHomePageFactory {
	...
	public void simpleFlight(String originCity, String destinationCity, String departureDate, String returnDate,
			String numOfAdults) throws Exception {
		clickFlightsTab();
		setOriginCity(originCity);
		setDestinationCity(destinationCity);
		setDepartureDate(departureDate);
		setReturnnDate(returnDate);
		selectAdultsOnFlight(numOfAdults);
		clickSearchButton();
		boolean isPresent = driver.findElements(By.xpath("//*[@id='acol-interstitial']/h3")).size() > 0;
		if (!isPresent) {
			log.info("Something went wrong in the simpleFlightTest");
